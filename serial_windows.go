...
func open(device string, baudRate int, dataBits byte, parity Parity, stopbits StopBits, readTimeout time.Duration) (p *Port, err error) {
	...
	h, err := syscall.CreateFile(name,
		...
		0)
	if err != nil {
		return nil, fmt.Errorf("open %v: %v", device, err)
	}
...
func setCommState(h syscall.Handle, baudRate int, dataBits byte, parity Parity, stopBits StopBits) error {
	...
	case ParitySpace:
		params.Parity = 4
	default:
		return fmt.Errorf("unsupported parity %c", parity)
	}
